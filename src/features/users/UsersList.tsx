...
const UsersList: React.FC = () => {
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const matchPerfil = u.perfil === 'Cliente';
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="font-poppins text-lg font-bold text-gray-800 dark:text-white">Gestão de Clientes (CRM)</h2>
          <p className="text-xs text-gray-500 mt-0.5">Monitore os perfis dos clientes, leads e histórico de interações.</p>
        </div>
      </div>

      {/* Filters bar */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white dark:bg-zinc-900 border border-gray-150 dark:border-zinc-800 p-4 rounded-2xl shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-zinc-800 font-sans">
                {filteredUsers.map((u) => (
                  <tr key={u.id} className="hover:bg-primary-medium/5 dark:hover:bg-zinc-800/80 transition-colors cursor-pointer" onClick={() => navigate(`/clientes/${u.id}`)}>
                    ...
                    <td className="px-6 py-4 text-right whitespace-nowrap space-x-1.5" onClick={(e) => e.stopPropagation()}>
                      <button
                        onClick={() => navigate(`/clientes/${u.id}`)}
